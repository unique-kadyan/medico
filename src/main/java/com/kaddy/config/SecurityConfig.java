...
public class SecurityConfig {
    ...
    public SecurityFilterChain productionFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.ignoringRequestMatchers("/api/auth/**"))
                .authorizeHttpRequests(auth -> auth.requestMatchers("/api/auth/admin/**").hasRole("ADMIN")
                        ...
                        .requestMatchers("/api/assignments").hasRole("ADMIN").requestMatchers("/api/assignments/{id}")
                        .hasRole("ADMIN").requestMatchers("/api/assignments/doctor/**")
                        .hasAnyRole("ADMIN", "DOCTOR", "NURSE").requestMatchers("/api/assignments/patient/**")
                        ...
                        .hasAnyRole("ADMIN", "DOCTOR", "NURSE", "LAB_TECHNICIAN", "PATIENT")

                        .requestMatchers("/api/files/**")
                        .hasAnyRole("ADMIN", "DOCTOR", "NURSE", "LAB_TECHNICIAN", "PHARMACIST", "RECEPTIONIST")
    ...
    public SecurityFilterChain developmentFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable()).authorizeHttpRequests(auth -> auth.requestMatchers("/api/auth/admin/**")
                .hasRole("ADMIN").requestMatchers("/api/auth/**").permitAll().requestMatchers("/actuator/health")
                .permitAll().requestMatchers("/swagger-ui/**", "/v3/api-docs/**", "/swagger-ui.html").permitAll()

                .requestMatchers("/api/pending-users/**")
                .hasAnyRole("ADMIN", "DOCTOR_SUPERVISOR", "NURSE_MANAGER", "NURSE_SUPERVISOR")

                .requestMatchers("/api/users/profile", "/api/users/me").authenticated().requestMatchers("/api/users")
                .hasRole("ADMIN").requestMatchers("/api/users/{id}").hasRole("ADMIN")
                ...
                .requestMatchers("/api/patients/patient-id/{patientId}")
                .hasAnyRole("ADMIN", "DOCTOR", "NURSE", "RECEPTIONIST").requestMatchers("/api/async/patients/**")
                .hasAnyRole("ADMIN", "DOCTOR", "NURSE", "RECEPTIONIST")
                ...
                .requestMatchers("/api/assignments").hasRole("ADMIN").requestMatchers("/api/assignments/{id}")
                .hasRole("ADMIN").requestMatchers("/api/assignments/doctor/**").hasAnyRole("ADMIN", "DOCTOR", "NURSE")
