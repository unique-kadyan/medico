...
/**
 * Async Medication Service with functional programming and multi-threading
 */
@Service
...
public class AsyncMedicationService {
    ...
    /**
     * Get all medications asynchronously with parallel processing
     */
    @Async("taskExecutor")
    ...
    /**
     * Get inventory alerts using parallel processing
     */
    @Async("taskExecutor")
    public CompletableFuture<InventoryAlerts> getInventoryAlertsAsync() {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            CompletableFuture<List<MedicationDTO>> lowStockFuture = CompletableFuture.supplyAsync(() ->
                filterMedications(allMedications, Medication::isLowStock),
                taskExecutor
            );

            CompletableFuture<List<MedicationDTO>> expiredFuture = CompletableFuture.supplyAsync(() ->
                filterMedications(allMedications, Medication::isExpired),
                taskExecutor
            );

            CompletableFuture<List<MedicationDTO>> expiringSoonFuture = CompletableFuture.supplyAsync(() ->
                filterMedications(allMedications, Medication::isExpiringSoon),
                taskExecutor
            );
            ...
            return new InventoryAlerts(
                lowStockFuture.join(),
                expiredFuture.join(),
                expiringSoonFuture.join()
            );
        }, taskExecutor);
    }

    /**
     * Process medication stock updates in batches
     */
    @Async("batchExecutor")
    @Transactional
    public CompletableFuture<List<MedicationDTO>> batchUpdateStockAsync(
            Map<Long, Integer> stockUpdates) {

        log.info("Async: Batch updating stock for {} medications", stockUpdates.size());

        return CompletableFuture.supplyAsync(() -> {
            List<CompletableFuture<MedicationDTO>> updateFutures = stockUpdates.entrySet()
                .stream()
                .map(entry -> updateStockAsync(entry.getKey(), entry.getValue()))
                .collect(Collectors.toList());
    ...
    /**
     * Update single medication stock asynchronously
     */
    private CompletableFuture<MedicationDTO> updateStockAsync(Long id, Integer quantity) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            log.debug("Stock updated for medication {}: {} -> {}",
                id, medication.getStockQuantity() - quantity, medication.getStockQuantity());
    ...
    public CompletableFuture<MedicationReport> generateReportAsync() {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            // Parallel computations for different metrics
            CompletableFuture<Long> totalMedicationsFuture = CompletableFuture.supplyAsync(
                () -> (long) allMedications.size(), taskExecutor);

            CompletableFuture<Long> activeMedicationsFuture = CompletableFuture.supplyAsync(
                () -> allMedications.parallelStream().filter(Medication::getActive).count(),
                taskExecutor);

            CompletableFuture<Integer> totalStockFuture = CompletableFuture.supplyAsync(
                () -> allMedications.parallelStream()
                    .mapToInt(Medication::getStockQuantity)
                    .sum(),
                taskExecutor);
            ...
            CompletableFuture<Long> lowStockCountFuture = CompletableFuture.supplyAsync(
                () -> allMedications.parallelStream().filter(Medication::isLowStock).count(),
                taskExecutor);

            CompletableFuture<Long> expiredCountFuture = CompletableFuture.supplyAsync(
                () -> allMedications.parallelStream().filter(Medication::isExpired).count(),
                taskExecutor);

            // Wait for all computations
            CompletableFuture.allOf(
                totalMedicationsFuture,
                activeMedicationsFuture,
                totalStockFuture,
                categoryDistFuture,
                lowStockCountFuture,
                expiredCountFuture
            ...
            return new MedicationReport(
                totalMedicationsFuture.join(),
                activeMedicationsFuture.join(),
                totalStockFuture.join(),
                categoryDistFuture.join(),
                lowStockCountFuture.join(),
                expiredCountFuture.join(),
                LocalDate.now()
            );
        }, taskExecutor);
    }
    ...
    /**
     * Search medications with multiple criteria using functional composition
     */
    @Async("taskExecutor")
    public CompletableFuture<List<MedicationDTO>> advancedSearchAsync(
            String name,
            String category,
            Boolean lowStock,
            Boolean expired) {

        log.info("Async: Advanced search - name: {}, category: {}, lowStock: {}, expired: {}",
            name, category, lowStock, expired);

        return CompletableFuture.supplyAsync(() -> {
            List<Medication> allMedications = medicationRepository.findAll();

            // Build composite predicate using functional composition
            Predicate<Medication> combinedFilter = med -> true; // Start with always true

            if (name != null && !name.isEmpty()) {
                combinedFilter = combinedFilter.and(
                    med -> med.getName().toLowerCase().contains(name.toLowerCase()));
            }

            if (category != null && !category.isEmpty()) {
                combinedFilter = combinedFilter.and(
                    med -> med.getCategory().equalsIgnoreCase(category));
            }
    ...
    /**
     * Bulk import medications asynchronously
     */
    @Async("batchExecutor")
    @Transactional
    public CompletableFuture<BulkImportResult> bulkImportMedicationsAsync(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            for (List<MedicationDTO> batch : batches) {
                try {
                    List<Medication> medications = batch.parallelStream()
                        .map(dto -> {
    ...
    /**
     * Automatic reorder processing for low stock items
     */
    @Async("batchExecutor")
    public CompletableFuture<List<ReorderSuggestion>> generateReorderSuggestionsAsync() {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            return lowStockMeds.parallelStream()
                .map(med -> new ReorderSuggestion(
                    med.getId(),
                    med.getName(),
                    med.getMedicationCode(),
                    med.getStockQuantity(),
                    med.getReorderLevel(),
                    med.getReorderQuantity(),
                    med.getUnitPrice().multiply(
                        java.math.BigDecimal.valueOf(med.getReorderQuantity())
                    )
                ))
                .collect(Collectors.toList());
        }, taskExecutor);
    ...
    private List<MedicationDTO> filterMedications(
            List<Medication> medications,
            Predicate<Medication> filter) {
    ...
    public record InventoryAlerts(
        List<MedicationDTO> lowStock,
        List<MedicationDTO> expired,
        List<MedicationDTO> expiringSoon
    ) {}

    public record MedicationReport(
